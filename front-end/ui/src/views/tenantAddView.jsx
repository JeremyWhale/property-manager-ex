...
export default function TenantAddView() {
    ...
    const handleComplete = () => {
        ...
        if (emergencyContactEmail === ''){
            ...
            axios.post(`${apiLocation}/tenant-add/`, data)
            .then(response => {
    ...
    function handleStepShow(){
        ...
        // Completed step
            ...
            return(
                <>
                    {uploadSuccess ? (
                        <StaticAlert type='success' message='New tenant successfully uploaded'/>
                    ) : (
                        <StaticAlert type='error' message='An error occurred when uploading tenant, tenant upload failed'/>
                    )}
                    <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
                        <Button onClick={handleReset} color="inherit">Add another tenant</Button>
                        <Box sx={{ flex: '1 1 auto' }} />
                        <Button href="/tenants">
