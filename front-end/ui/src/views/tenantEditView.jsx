...
export default function TenantEditView() {
    ...
    const [emergencyContactEmailError, setEmergencyContactEmailError] = useState(false);
    ...
    useEffect(() => {
        async function getTenant(){
            ...
            const response = await axios.get(`${apiLocation}/tenant-details/${tenant}`);
    ...
    const handleComplete = () => {
        ...
        if (emergencyContactEmail === ''){
            ...
            axios.put(`${apiLocation}/tenant-edit/${id}`, data)
            .then(response => {
    ...
    async function handleDelete(){
        await axios.delete(`${apiLocation}/tenant-delete/${id}`)
        .then((response) => {
            setIsDelete(true)
            setUploadSuccess(true)
            setActiveStep(3)
    ...
    function handleStepShow(){
        ...
        // Completed step
            ...
            return(
                <>
                    {uploadSuccess ? (
                        isDelete ? <StaticAlert type='success' message='Tenant successfully deleted'/> : <StaticAlert type='success' message='Tenant successfully edited'/>
                    ) : (
                        <StaticAlert type='error' message='An error occurred when editing tenant, tenant edit failed'/>
                    )}
                    <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
                        <Box sx={{ flex: '1 1 auto' }} />
                        <Button href="/tenants">
