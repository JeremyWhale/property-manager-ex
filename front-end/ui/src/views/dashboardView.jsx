...
export default function DashboardView(){
    ...
    const [numberOfElectricalInspectionsdue, setNumberOfElectricalInspectionsDue] = useState(0)
    const [electricalInspectionsModalOpen, setElectricalInspectionsModalOpen] = useState(false)
    ...
    const [numberOfInsuranceRenewalsdue, setNumberOfInsuranceRenewalsDue] = useState(0)
    const [insuranceRenewalsdueModalOpen, setInsuranceRenewalsdueModalOpen] = useState(false)

    const [rentRenewalsdue, setRentRenewalsDue] = useState()
    const [numberOfRentRenewalsdue, setNumberOfRentRenewalsDue] = useState(0)
    const [rentRenewalsdueModalOpen, setRentRenewalsdueModalOpen] = useState(false)

    const [tenancyRenewalsdue, setTenancyRenewalsDue] = useState()
    const [numberOfTenancyRenewalsdue, setNumberOfTenancyRenewalsDue] = useState(0)
    const [tenancyRenewalsdueModalOpen, setTenancyRenewalsdueModalOpen] = useState(false)

    const [mortgageRenewalsdue, setMortgageRenewalsDue] = useState()
    const [numberOfMortgageRenewalsdue, setNumberOfMortgageRenewalsDue] = useState(0)
    const [mortgageRenewalsdueModalOpen, setMortgageRenewalsdueModalOpen] = useState(false)

    const [councilLicenseRenewalsdue, setCouncilLicenseRenewalsDue] = useState()
    const [numberOfCouncilLicenseRenewalsdue, setNumberOfCouncilLicenseRenewalsDue] = useState(0)
    const [councilLicenseRenewalsdueModalOpen, setCouncilLicenseRenewalsdueModalOpen] = useState(false)
    ...
    //Gas certs
    useEffect(() => {
        axios.get(`${apiLocation}/gas-certificates-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    //EPC certs
    useEffect(() => {
        axios.get(`${apiLocation}/epc-certificates-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    //Rent reviews
    useEffect(() => {
        axios.get(`${apiLocation}/rent-reviews-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    //Tenancy reviews due
     useEffect(() => {
        axios.get(`${apiLocation}/tenancy-reviews-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    useEffect(() => {
        axios.get(`${apiLocation}/electrical-inspections-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    //Insurance renewals
    useEffect(() => {
        axios.get(`${apiLocation}/insurance-renewals-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    //Mortgage renewals
    useEffect(() => {
        axios.get(`${apiLocation}/mortgage-renewals-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    useEffect(() => {
        axios.get(`${apiLocation}/council-license-renewals-due/90`)
            .then(response => {
                const data = response.data;
                const mappedRows = []
                data.map(property => (mappedRows.push({
    ...
    return(
        <>
            <IcPageHeader heading={`Welcome to the Property Manager, ${name}`} />
            ...
            {/* Gas */}
            <Modal open={gasCertsModalOpen && numberOfGasCertsdue !== 0} onClose={() => setGasCertsModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> Gas Certificates Due (within 3 months)</Typography>
                    <GasCertModal data={gasCertsdue} />
                </Box>
            </Modal>
            {/* EPC */}
            <Modal open={epcsModalOpen && numberOfEpcsdue !== 0} onClose={() => setEpcsModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> EPCs Due (within 3 months)</Typography>
                    <GasCertModal data={epcsdue} />
                </Box>
            </Modal>
            {/* Rent Renews */}
            <Modal open={rentRenewalsdueModalOpen && numberOfRentRenewalsdue !== 0} onClose={() => setRentRenewalsdueModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> Rent Renewals Due (within 3 months)</Typography>
                    <GasCertModal data={rentRenewalsdue} />
                </Box>
            </Modal>
            {/* Tenancy renrews */}
            <Modal open={tenancyRenewalsdueModalOpen && numberOfTenancyRenewalsdue !== 0} onClose={() => setTenancyRenewalsdueModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> Tenancy Renewals Due (within 3 months) </Typography>
                    <GasCertModal data={tenancyRenewalsdue} />
                </Box>
            </Modal>
            {/* Elect */}
            <Modal open={electricalInspectionsModalOpen && numberOfElectricalInspectionsdue !== 0} onClose={() => setElectricalInspectionsModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> Electrical Inspections Due (within 3 months)</Typography>
                    <GasCertModal data={electricalInspectionsdue} />
                </Box>
            </Modal>
            {/* Insure */}
            <Modal open={insuranceRenewalsdueModalOpen && numberOfInsuranceRenewalsdue !== 0} onClose={() => setInsuranceRenewalsdueModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> Insurance Renewals Due (within 3 months)</Typography>
                    <GasCertModal data={insuranceRenewalsdue} />
                </Box>
            </Modal>
            {/* Mortgage */}
            <Modal open={mortgageRenewalsdueModalOpen && numberOfMortgageRenewalsdue !== 0} onClose={() => setMortgageRenewalsdueModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> Mortgage Renewals Due (within 3 months)</Typography>
                    <GasCertModal data={mortgageRenewalsdue} />
                </Box>
            </Modal>
            {/* Council L */}
            <Modal open={councilLicenseRenewalsdueModalOpen && numberOfCouncilLicenseRenewalsdue !== 0} onClose={() => setCouncilLicenseRenewalsdueModalOpen(false)}>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 2, borderRadius: 4 }}>
                    <Typography id="modal-modal-title" variant="h6" component="h2"> Council License Renewals Due (within 3 months)</Typography>
