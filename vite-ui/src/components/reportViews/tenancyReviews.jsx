...
export default function TenancyReviewsTable(props) {
  ...
  useEffect(() => {
    axios.get(`${apiLocation}/tenancy-reviews-due/${props.range}/`)
      .then(response => {
  ...
  if(rows.length === 0){
    ...
    return(
        <StaticAlert type='success' message={`No Tenancy Reviews due within ${rangeMonths} month(s)`} />
    )
  }
