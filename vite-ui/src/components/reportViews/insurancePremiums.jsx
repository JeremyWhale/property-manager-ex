...
export default function InsurancePremiumsTable(props) {
  ...
  useEffect(() => {
    axios.get(`${apiLocation}/insurance-renewals-due/${props.range}/`)
      .then(response => {
  ...
  if(rows.length === 0){
    ...
    return(
        <StaticAlert type='success' message={`No Insurance Renewals due within ${rangeMonths} month(s)`} />
    )
  }
              ...
              {visibleRows.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">£{row.currentPremium}</TableCell>
                    </TableRow>
                  );
                })}
                {emptyRows > 0 && (
