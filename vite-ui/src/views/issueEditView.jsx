...
export default function IssueEditView() {
    ...
    useEffect(() => {
        async function getIssue(){
            ...
            if (response.data.date_fixed === '2000-01-02' || response.data.date_fixed === '2000-01-01'){
    ...
    async function handleDelete(){
        await axios.delete(`${apiLocation}/issue-delete/${id}`)
        .then((response) => {
            setIsDelete(true)
    ...
    const handleNext = () => {
        {!beenAllocated && (
            setDateFixed('2000-01-01')
        )}
        {!beenAllocated && (
            setSelectedContractorName(1)
        )}
    ...
    const handleComplete = () => {
        ...
        if(selectedContractorName !== 0){
            ...
            axios.put(`${apiLocation}/issue-edit/${id}`, data)
            .then(response => {
                setUploadSuccess(true)
            })
            .catch(error => {
            ...
            if(accountNumber === ''){
                ...
                axios.post(`${apiLocation}/contractor-add/`, contractorData)
                .then(response => {
    ...
    function handleStepShow(){
                    ...
                    <Grid container spacing={2} sx={{paddingTop: 2}}>
                        ...
                        <Grid item xs={12}>
                            <FormControlLabel
                                ...
                                onClick={() => {setBeenAllocated(!beenAllocated)}}
                            />
                        </Grid>
                        {beenAllocated && (
                        <Grid item xs={12}>
                            <FormControlLabel
                                ...
                                labelPlacement="start"
                                onClick={() => {setBeenFixed(!beenFixed)}} 
                            />
                            {beenFixed && (
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DatePicker 
                        ...
                        <Grid item xs={12}>
                            ...
                            >
                                {selectedContractorName !== 0 && (
                                        <MenuItem key={''} value={0}>New Contractor</MenuItem>
                                    )}
        ...
        // Completed step
            ...
            return(
                <>
                    {uploadSuccess ? (
                        isDelete ? <StaticAlert type='success' message='Issue successfully deleted'/> : <StaticAlert type='success' message='Issue successfully edited'/>
                    ) : (
                        <StaticAlert type='error' message='An error occurred when editing issue, issue edit failed'/>
                    )}
                    <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
                        <Box sx={{ flex: '1 1 auto' }} />
                        <Button href="/issues">
