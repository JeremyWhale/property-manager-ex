...
export default function TenantView(){
    ...
    useEffect(() => {
        async function getTenantDetails() {
            try {
                const response = await axios.get(`${apiLocation}/tenant-details/${selectedTenant}`);
    ...
    useEffect(() => {
        async function getTenancyDetails() {
            if (tenantDetails && tenantDetails.addressLine1 !== 'No Property Listed'){
                try {
                    const response = await axios.get(`${apiLocation}/tenancy-details/${selectedTenant}`);
    ...
    return(
        <>
            ...
            reverseOrder>
            <IcButton slot="actions" variant="primary" href="/tenants/add">
                <Add slot="left-icon"/> Add Tenant
            </IcButton>
            {selectedTenant.length === 0 ? (<></>) :(
                <IcButton slot="actions" variant="tertiary" href={`/tenants/edit?tenant=${selectedTenant}`}>
        ...
        {selectedTenant !== '' && moveInDate !== undefined && tenantDetails && address !== undefined ? (
            <Grid container rowSpacing={2} sx={{paddingTop: 2}}>
                {/* Row 1 */}
                <Grid item xs={4}>
                    <Typography><b>Full name: </b>{selectedTenant}</Typography>
                </Grid>
                <Grid item xs={8}>
                    {address === 'No Property Listed' ? <Typography><b>Property: </b>{address}</Typography> : <Typography><b>Property: </b><a href={`/properties?property=${address}`}>{address}</a></Typography>}
                </Grid>
                {/* Row 2 */}
                <Grid item xs={4}>
                    <Typography><b>Phone Number: </b>{tenantDetails.phoneNumber}</Typography>
                </Grid>
                <Grid item xs={8}>
                    <Typography><b>Move in date: </b>{moveInDate}</Typography>
                </Grid>
                {/* Row 3 */}
                <Grid item xs={4}>
                    <Typography><b>Email: </b>{tenantDetails.email}</Typography>
                </Grid>
                <Grid item xs={8}>
                    <Typography><b>Emergency Contact Name: </b>{tenantDetails.EContactName}</Typography>
                </Grid>
                {/* Row 4 */}
                <Grid item xs={4}>
                    <Typography><b>Sort Code: </b>{tenantDetails.sortCode}</Typography>
                </Grid>
                <Grid item xs={8}>
                    <Typography><b>Emergency Contact Phone Number: </b>{tenantDetails.EContactNumber}</Typography>
                </Grid>
                {/* Row 5 */}
                <Grid item xs={4}>
                    <Typography><b>Account number: </b>{tenantDetails.accountNumber}</Typography>
                </Grid>
                <Grid item xs={8}>
                    <Typography><b>Emergency Contact Email: </b>{tenantDetails.EContactEmail}</Typography>
                </Grid>
            </Grid>
        ) : 

        (<StaticAlert type='info' message='Select a tenant to view their details'/>)}
    </>
    )
}