...
export default function IssueAddView() {
    ...
    const handleComplete = () => {
        ...
        if(selectedContractorName !== 0){
            ...
            axios.post(`${apiLocation}/issue-add/`, data)
            .then(response => {
    ...
    function handleStepShow(){
        ...
        // Completed step
            ...
            return(
                <>
                    {uploadSuccess ? (
                        <StaticAlert type='success' message='New issue successfully uploaded'/>
                    ) : (
                        <StaticAlert type='error' message='An error occurred when uploading issue, issue upload failed'/>
                    )}
                    <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
                        <Button onClick={handleReset} color="inherit">Add another issue</Button>
                        <Box sx={{ flex: '1 1 auto' }} />
                        <Button href="/issues">
